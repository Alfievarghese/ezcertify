...
export const ColorPicker = ({
  ...
  useEffect(() => {
    if (value) {
      try {
        ...
        setAlpha(color.a !== undefined ? color.a * 100 : 100);
      } catch (e) {
  ...
  useEffect(() => {
    if (onChange) {
      ...
      const hex = color.hex(); // usually it's better to pass hex to canvas
      
      // We pass the string representation instead of the raw array so we don't break existing components.
      onChange(hex as any);
    }
...
export const ColorPickerFormat = ({
  ...
  if (mode === 'hex') {
    ...
    return (
      <div
        className={cn(
          '-space-x-px relative flex items-center shadow-sm w-full',
          className
        )}
        {...props}
      >
